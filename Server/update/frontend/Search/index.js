...
const Search = ({ route, navigation }) => {
    ...
    useEffect(()=>{
        async function getData(){
            ...
            var result = await axios.get(`${SERVER_ADDRESS}/front-end/videoRecommendation/cbdef4d4-7168-11eb-a09f-f0795907d9ec`)
                            ...
                            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                                {randomize(data[categorie]).map((video, key) => (
                                    <TouchableHighlight key={key} onPress={() => {
                                        navigation.navigate("Video", {videoId:video.video_id})
                                    }}>
                                        <ImageBackground
                                            key={key}
                                            source={{uri:video.creator.imageUri}}
                                            style={{
                                                width: 145,
                                                height: 200,
                                                marginHorizontal: 1,
